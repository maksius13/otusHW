...
public class Assertions {
    ...
    public static void assertBetween(int start, int finish, int check) {
        if (check < start || check > finish){
            throw new AssertionError(String.format("Expected \"%s\" <= \"%s\" <= \"%s\"", start, check,finish));
        }
    }

    public static void assertNotContain(String find, List<String> list){
        if (list.contains(find)){
            throw new AssertionError(String.format("Expected \"%s\" missing in %s", find, list));
        }
    }
